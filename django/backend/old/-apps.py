...
class SuitConfig(DjangoSuitConfig):
    menu = (
        ParentItem('Content', children=[
            ChildItem(model='backend.country'
            # , params={'o': '1.2'}
            ),
            ChildItem(model='backend.continent'),
            ChildItem(model='backend.showcase'),
            ChildItem('Custom view', url='/admin/custom/'),
        ], icon='fa fa-leaf'),
        ParentItem('Integrations', children=[
            ChildItem(model='backend.city'),
        ]),
    ...
    def ready(self):
        ...
        # It is only to prevent updating last_login in DB for backend app
